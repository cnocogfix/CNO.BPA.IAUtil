   ...
   public class DataAccess
   {
      ...
      public void createBatchProcedure( INodeValueProvider nodeValueProvider,INode node, string ActionIndicator, Dictionary<string, string> Fields)
      {
            try
            {
               ...
               using (OracleCommand cmd = GenerateCommand(CREATE_BATCH,CommandType.StoredProcedure))
               {
                    cmd.BindByName = true;
                    DBUtilities.CreateAndAddParameter("p_in_action_indicator",
                     ActionIndicator, OracleDbType.Varchar2, ParameterDirection.Input, cmd);
                  ...
                  foreach (KeyValuePair<string, string> fm in Fields)
                  {
                     if (fm.Value.Length > 0)
                     {
                        if (fm.Value.Contains("$instance"))
                        {
                           ...
                           if (parameterValue.Trim().Length != 0)
                           {
                              ...
                              DBUtilities.CreateAndAddParameter(fm.Key,
                                 parameterValue.ToUpper(), OracleDbType.Varchar2,
                        ...
                        {
                           DBUtilities.CreateAndAddParameter(fm.Key,
                              fm.Value.ToUpper(), OracleDbType.Varchar2,
                              ParameterDirection.Input, cmd);
                        }
                     }
                        else
                        {
                            //parameterValue = Convert. DBNull.Value; setting DBnull to the params [21.4]

                            DBUtilities.CreateAndAddParameter(fm.Key,
                             DBNull.Value, OracleDbType.Varchar2, ParameterDirection.Input, cmd);
               ...
               CommandType.StoredProcedure))
            {
               ...
               foreach (KeyValuePair<string, string> fm in Fields)
               {
                  ...
                  if (Value.Length > 0)
                  {
                     ...
                     {
                        DBUtilities.CreateAndAddParameter(fm.Key,
                           Value.ToUpper(), OracleDbType.Varchar2,
                           ParameterDirection.Input, cmd);
                     }
                  }
               }

               DBUtilities.CreateAndAddParameter("p_out_item_id", OracleDbType.Varchar2,
               ...
               CommandType.StoredProcedure))
            {
               ...
               if (batchNoParameter.Trim().Length != 0)
               {
                  DBUtilities.CreateAndAddParameter("P_IN_BATCH_NO",
                     batchNoParameter.ToUpper(), OracleDbType.Varchar2,
                     ParameterDirection.Input, cmd);
               }
         ...
         try
         {
            ...
            string parameterValue = "";
                   ...
                   CommandType.StoredProcedure))
                {
                    ...
                    DBUtilities.CreateAndAddParameter("p_out_ia_statistics_id", OracleDbType.Varchar2,
                       ParameterDirection.Output, 25, cmd);
                    DBUtilities.CreateAndAddParameter("p_out_result", OracleDbType.Varchar2,
                       ParameterDirection.Output, 255, cmd);
                    DBUtilities.CreateAndAddParameter("p_out_error_message", OracleDbType.Varchar2,
      ...
      public void createBatchError(INodeValueProvider nodeValueProvider, INode node, Dictionary<string, string> Fields)
      {
         try
         {
               ...
               CommandType.StoredProcedure))
            {
               ...
               DBUtilities.CreateAndAddParameter("p_out_batch_err_id", OracleDbType.Varchar2,
                  ParameterDirection.Output, 25, cmd);
               DBUtilities.CreateAndAddParameter("p_out_result", OracleDbType.Varchar2,
                  ParameterDirection.Output, 255, cmd);
               DBUtilities.CreateAndAddParameter("p_out_error_message", OracleDbType.Varchar2,
